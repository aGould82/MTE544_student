...
class planner:
    ...
    def plan(self, goalPoint=[-3.0, -3.0]):
    ...
    def trajectory_planner(self):
        ...
        # return 
        #x_vals = np.linspace(0, 1.5, 100) #get x values
        #for x in x_vals:
        #    y = x ** 2  #map x values to y values for a parabola
        #    traj_points.append([x, y]) #append the points to trajectory points list

        
        x_vals = np.linspace(0, 2.5, 100)#get x values
        for x in x_vals:
            y = 2.0 / (1.0 + math.exp(-2.0 * x))-1 #map x values to y values for a sigmoid
            traj_points.append([x, y]) #append the points to trajectory points list
        return traj_points
