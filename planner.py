...
class planner:
    ...
    def trajectory_planner(self, startPoseCart, endPoseCart):
        ...
        startPose = self.m_utilites.position_2_cell(startPoseCart) # convert to cell coordinates
        endPose = self.m_utilites.position_2_cell(endPoseCart) # convert to cell coordinates

        # convert to tuple
        startPose = (startPose[0], startPose[1])
        endPose = (endPose[0], endPose[1])
        # TODO: Call the A* search algorithm
        path = search(self.costMap, startPose, endPose) # perform A* search
        if path is None:
            return None
