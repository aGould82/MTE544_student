...
class localization(Node):
    ...
    def odom_and_pf_pose_callback(self, odom_msg: odom, pf_msg: odom):
        ...
        pf_x = pf_msg.pose.pose.position.x #get x position from particle filter message
        pf_y = pf_msg.pose.pose.position.y #get y position from particle filter message
        pf_th = euler_from_quaternion(pf_msg.pose.pose.orientation) #get theta from particle filter message
        self.pose = [pf_x, pf_y, pf_th, pf_msg.header.stamp] #update robot pose with pf values
        ...
        odom_x = odom_msg.pose.pose.position.x #get x position from odom message
        odom_y = odom_msg.pose.pose.position.y #get y position from odom message
        odom_th = euler_from_quaternion(odom_msg.pose.pose.orientation) #get theta from odom message
        odom_vx = odom_msg.twist.twist.linear.x #get linear velocity from odom message
        odom_yawrate = odom_msg.twist.twist.angular.z #get angular velocity from odom message
        odom_values_list = [odom_x, odom_y, odom_th, odom_vx, odom_yawrate] #update odom values list
        # TODO: pf values: x, y, theta
        pf_values_list = [pf_x, pf_y, pf_th] #update pf values list
