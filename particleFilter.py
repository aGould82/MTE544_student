...
class particleFilter(Node):
    ...
    def initializeParticleFilter(self, x, y, th):
        ...
        xs = np.random.normal(loc=x, scale=self.std_particle_x, size=numParticles) #generate x positions around initial x
        ys = np.random.normal(loc=y, scale=self.std_particle_y, size=numParticles) #generate y positions around initial y
        ths = np.random.normal(loc=th, scale=self.std_particle_theta, size=numParticles) #generate thetas around initial th
        
        
        ths = (ths + np.pi) % (2 * np.pi) - np.pi #wrap angles between -pi and pi

        self.particlePoses = np.vstack((xs, ys, ths)).T #size should be (numParticles, 3)
    ...
    def resample(self, laser_scan, mapUtilInstance):
        ...
        N = len(self.particles) #get current number of particles
        indices = np.random.choice(np.arange(N), size=N, replace=True, p=particles_weights) #randomly choose particle indices based on weights
        sampled_particles = [self.particles[i] for i in indices] #get the sampled particles
        

        for bp in sampled_particles:
            x, y, th = bp.getPose()
            # TODO: add noise to the x, y, and th, use the same std_noise for x, y, and th
            new_x = x + np.random.normal(0.0, std_noise) #randomly add noise to x
            new_y = y + np.random.normal(0.0, std_noise) #randomly add noise to y
            new_th = th + np.random.normal(0.0, std_noise) #randomly add noise to theta
