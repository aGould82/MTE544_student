...
def search(maze, start, end):
    ...
    start_node = Node(parent=None, position=start) # create start node
    start_node.g = 0.0     # cost from start Node
    # heuristic estimated cost to end Node
    #start_node.h = sqrt((start[0] - end[0])**2 + (start[1] - end[1])**2) # Euclidean Distance 
    start_node.h = (start[0]-end[0]+start[1]-end[1]) # Manhattan Distance
    start_node.f = start_node.g + start_node.h # total cost

    end_node = Node(parent=None, position=end) # create end node
    end_node.g = float('inf') # set a large value if not defined
    end_node.h = 0.0 # heuristic estimated cost to end Node
    end_node.f = end_node.g + end_node.h # total cost
    ...
    while len(yet_to_visit_dict) > 0:
        ...
        for position, node in yet_to_visit_dict.items():
            if current_fscore is None or node.f < current_fscore:
                current_fscore = node.f # get lowest f value
                current_node = node # get node associated with the lowest f value
        ...
        for new_position in move:
            ...
            if (node_position[0] > (no_rows - 1) or node_position[0] < 0 or 
                node_position[1] > (no_columns - 1) or node_position[1] < 0): # check if within range
                continue
        ...
        for child in children:
            ...
            if (visited_dict.get(child.position, False)): # if child is already visited
                continue

            # TODO PART 4 Create the f, g, and h values
            child.g = current_node.g + 1 # assuming cost between nodes is 1
            # Heuristic costs calculated 
            #child.h = sqrt((child.position[0] - end_node.position[0])**2 + (child.position[1] - end_node.position[1])**2)# Euclidean Distance
            child.h = (child.position[0]-end_node.position[0]+child.position[1]-end_node.position[1]) # Manhattan Distance
